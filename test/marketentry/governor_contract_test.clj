  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Honiara Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Honiara Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest certificate-scope-exceeded-is-held-and-unoverridable
  (testing "declared :claims-certificate-covers-activity? true but the declared activity is not on the certificate's own list -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:certificate-scope-exceeded} (-> (store/ledger db) last :basis)))
...
(deftest tin-record-unverified-is-held-and-unoverridable
  (testing "unverified IRD TIN record when required -> HARD hold"
    (let [[db actor] (fresh)
