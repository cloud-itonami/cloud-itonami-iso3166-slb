(ns marketentry.marketentryllm
  ...
  the Solomon Islands (SLB) public-sector market-entry compliance
  actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "claims-certificate-covers-activity?=" (:claims-certificate-covers-activity? e)
                        " declared-activity=" (:declared-activity e)
